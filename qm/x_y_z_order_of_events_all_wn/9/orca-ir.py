...
w = 7.5                                  #w = line width for broadening, FWHM
wn_add = 25                             #add +150 to spectra x (required for convolution)
    ...
    with open(args.filename, "r") as input_file:
        for line in input_file:
            ...
            if "Program Version 6" in line:
                #thanks to the orca prgrmrs intensity is now in a different column it is the EPS column
                intens_column=2
...
#plt_range_x=np.arange(750.2678073,max(freqlist)+wn_add,1.928709)
...
intens_mat=pandas.DataFrame(intenslist)
freq_mat=pandas.DataFrame(freqlist)
intens_mat.index=freq_mat.values.reshape(-1)

pandas.DataFrame(gauss_sum).to_csv("tmp.csv")
pandas.DataFrame(intens_mat).to_csv("intenslist.csv")
pandas.DataFrame(freqlist).to_csv("freqlist.csv")
