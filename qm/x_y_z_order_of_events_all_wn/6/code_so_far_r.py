...
df_rcoords = np.sqrt(df_zcoords.iloc[:,1]*df_zcoords.iloc[:,1]+df_zcoords.iloc[:,2]*df_zcoords.iloc[:,2])

#c=normalized_df.multiply(df_zcoords.iloc[:,3], axis=0)  ##normalized_df
c=normalized_df.multiply(df_rcoords.values, axis=0)  ##normalized_df
...
#oe=order_events(cm)
#oe.to_csv('order_of_events.csv')



file_path='setup_1.out'
#modify_numbers_in_file(file_path,var)
os.system("python orca-ir.py freq.out")
#os.system("python remove_850.py")
#gauss_sum = list()
#orcair(modelist, var, intenslist, gauss_sum )
#The convolution occured above
ranks = pd.read_csv('ranked_modes_1.csv',header=0, index_col=0)
gauss = pd.read_csv('intenslist.csv',header=0, index_col=0)
#mr=ranks[14:414].values
...
#input_2d=input_2d.drop(index=range(1800,2750))
#input_2d=input_2d.drop(index=range(1475,1620))

#int_dist=int_dist.drop(index=range(1800,2750))
#int_dist=int_dist.drop(index=range(1475,1620))
input_2d.to_csv('2t2d_input_r.csv')

os.system("python 2Dpy_r.py")

spec1 = pandas.read_csv('ref_gaussians_r.csv', header=0, index_col=0).T
int_dist=int_dist.iloc[1:len(int_dist)]
int_dist.index=spec1.T.index

out_count_dist=pd.concat([spec1.T, int_dist], axis=1)
out_count_dist.to_csv('count_dist_r.csv')

#y_sim=order_events(summation_ranked_wn)
