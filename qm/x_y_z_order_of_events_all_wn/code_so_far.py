...
df_zcoords=df_zcoords.iloc[:,3]+40
...
input_2d.to_csv('2t2d_input.csv')

os.system("python 2Dpy.py")

spec1 = pandas.read_csv('ref_gaussians.csv', header=0, index_col=0).T
int_dist=int_dist.iloc[1:len(int_dist)]
int_dist.index=spec1.T.index

out_count_dist=pd.concat([spec1.T, int_dist], axis=1)
out_count_dist.to_csv('count_dist.csv')
