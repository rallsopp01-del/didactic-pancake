...
for i in {1..50..1}; do
	for j in  {51..100..1}; do
    		mkdir $i$j
		cp ../x_y_z_order_of_events_all_wn/$i/count_dist_z.csv ./$i$j/count_dist_a.csv
		cp ../x_y_z_order_of_events_all_wn/$i/count_dist_r.csv ./$i$j/count_dist_a_r.csv
		cp ../$i/freq.xyz ./$i$j/freq.QMRegion_a.xyz
		cp ../x_y_z_order_of_events_all_wn/$j/count_dist_z.csv ./$i$j/count_dist_b.csv
		cp ../x_y_z_order_of_events_all_wn/$j/count_dist_r.csv ./$i$j/count_dist_b_r.csv
		cp ../$j/freq.xyz ./$i$j/freq.QMRegion_b.xyz
		#This is for RMSD
		cp z_r_rmsd.tcl ./$i$j/z_r_rmsd.tcl
		cp knn.py ./$i$j/knn.py
		cp fit.py ./$i$j/fit.py
		cd ./$i$j/
		#This is for z-r-RMSD
		vmd -e z_r_rmsd.tcl -args $i $j
		...
		awk -F, 'BEGIN {OFS=","} NR==1 {print $1, $2} NR>1 {new_value = 402 - $2; print $1, new_value}' temp_b.csv > temp_b_flip.csv	
